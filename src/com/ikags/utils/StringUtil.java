...
public class StringUtil {
	...
	public static void printLong(String data) {
		try{
			int sizedata=1000;
		if (data != null && data.length() > sizedata) {
			int size = (data.length() / sizedata) ;
			for (int i = 0; i <= size; i++) {
				//System.out.println(data.length() +","+size+","+i+"==============="+i);
				if(i==size){
					System.out.println(data.substring(i*sizedata, data.length()));
				}else{
					System.out.println(data.substring(i*sizedata, (i+1)*sizedata));
				}
			}
		} else {
	...
	public static String getLocalFileText(String filepath) {
		// 读取数据
		String newdata = null;
		try {
			...
			newdata = StringUtil.getInputStreamText(in, "UTF-8");
	...
	/**
	 * 数据截取：例子
	 * String data= getTextContent(data, "/>", "</");
	 * @param rawdata
	 * @param startstr
	 * @param endstr
	 * @return
	 */
	public static String getTextContent(String rawdata, String startstr, String endstr) {
		String info = "";
		try {
			int satrtindex = rawdata.indexOf(startstr);
			int endindex = rawdata.indexOf(endstr, satrtindex);

			if (satrtindex >= 0) {
				info = rawdata.substring(satrtindex + startstr.length(), endindex).trim();
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
